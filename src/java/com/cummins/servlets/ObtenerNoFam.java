 ...
 * @author nc784
 */
public class ObtenerNoFam extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            System.out.println("Consiguiendo Obtener No Familiarizado");
            session = request.getSession();
            conn = Conexion.getDS().getConnection();
            conn.setAutoCommit(false);
            jsonCapacitaJefeTaller.setData(obtenerNoFam());
            mapper = new ObjectMapper();
            conn.commit();
        } catch (SQLException sqlEx) {
            System.out.println("ObtenerNoFam::processRequest SQLException: " + sqlEx);
        } catch (NumberFormatException numEx) {
            System.out.println("ObtenerNoFam::processRequest NumberFormatException: " + numEx);
        } catch (Exception e) {
            System.out.println("ObtenerNoFam::processRequest Exception: " + e);
        } finally {
            if (conn != null) {
                try {
                    conn.close();
    ...
    public ArrayList<Data> obtenerNoFam() throws Exception {
        ArrayList<Data> alListaMatriz = new ArrayList<Data>();
        CallableStatement csObtenerMatriz = null;
        ResultSet rsListaMatriz = null;
        Data data;
        System.out.println("Ejecutando Procedimiento ObtenerNoFam");
        csObtenerMatriz = conn.prepareCall("{ call ZMKT.ZMKT7257_DES_TEST_PKG.OBT_NO_FAMILIARIZADO(?)}");
