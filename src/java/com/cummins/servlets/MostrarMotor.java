 ...
 * @author nc784
 */
public class MostrarMotor extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            System.out.println("Consiguiendo Lista Mostrar Motor");
            session = request.getSession();
            conn = Conexion.getDS().getConnection();
            conn.setAutoCommit(false);
            jsonCapacitaJefeTaller.setData(obtenerMotor());
            mapper = new ObjectMapper();
            conn.commit();
        } catch (SQLException sqlEx) {
            System.out.println("MostrarMotor::processRequest SQLException: " + sqlEx);
        } catch (NumberFormatException numEx) {
            System.out.println("MostrarMotor::processRequest NumberFormatException: " + numEx);
        } catch (Exception e) {
            System.out.println("MostrarMotor::processRequest Exception: " + e);
        } finally {
            if (conn != null) {
                try {
                    conn.close();
    ...
    public ArrayList<Data> obtenerMotor() throws Exception {
        ArrayList<Data> alListaMatriz = new ArrayList<Data>();
        CallableStatement csObtenerMatriz = null;
        ResultSet rsListaMatriz = null;
        Data data;
        System.out.println("Ejecutando Procedimiento Mostrar Motor");
        csObtenerMatriz = conn.prepareCall("{ call ZMKT.ZMKT7257_DES_TEST_PKG.CAPA_JT_MOSTRAR_MOTOR(?)}");
        ...
        while (rsListaMatriz.next()) {
            ...
            data.setMotor(rsListaMatriz.getString("MATRIZ_NAME"));
